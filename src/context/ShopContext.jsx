...
export const ShopProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const cartSession = {
      id: sessionId, userEmail, userName, userPhone, cart,
      timestamp: Date.now(), status: 'PENDING', emailSent: false, recoveryUrl
    };
  ...
  // 3. 1-Hour Timer Check — Sends Automated Email if cart abandoned > 1 hour
  useEffect(() => {
    const checkAndDispatch = async () => {
      try {
        ...
        if (ageMs >= 3600000 && session.userEmail) {
          try {
